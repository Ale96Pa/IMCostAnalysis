def calculateMissing(dfMiss):
    ...
    if dfMiss["missingA"][0] == True:
        countMiss+=1
    if dfMiss["missingK"][0] == True:
        countMiss+=1
    if dfMiss["missingD"][0] == True:
        countMiss+=1
    if dfMiss["missingY"][0] == True:
        countMiss+=1
...
def calculateMultiple(dfMult, numEv):
    tot = (int(dfMult["multipleN"][0])/numEv) + (int(dfMult["multipleA"][0])/numEv) 
    + (int(dfMult["multipleW"][0])/numEv) + (int(dfMult["multipleG"][0])/numEv) 
    + (int(dfMult["multipleR"][0])/numEv) + (int(dfMult["multipleC"][0])/numEv)
